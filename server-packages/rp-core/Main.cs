...
namespace RPCore
{
    public class Main
    {
        ...
        private async void InitializeDatabase()
        {
            ...
            Console.WriteLine("[RP-CORE] Teste Datenbankverbindung...");
            bool connected = await DatabaseManager.Instance.TestConnection();

            if (connected)
            {
                Console.WriteLine("[RP-CORE] ✓ Datenbankverbindung erfolgreich");
            }
            else
            {
                Console.WriteLine("[RP-CORE] ✗ WARNUNG: Datenbankverbindung fehlgeschlagen!");
                Console.WriteLine("[RP-CORE]    Überprüfe configs/database.json und MySQL Server");
            }
        }
